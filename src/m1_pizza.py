...
def run_test_generate_points_on_circle():
    ...
    # ------------------------------------------------------------------
    expected = [rg.Point(185.0, 40.0),
                rg.Point(160.8, 73.3),
                rg.Point(121.7, 60.6),
                rg.Point(121.7, 19.4),
                rg.Point(160.8, 6.7)]
    circle = rg.Circle(rg.Point(150, 40), 35)
...
def run_test_draw_points_on_circle():
    ...
    # ------------------------------------------------------------------
    # Test 1:
    title = 'DRAW_POINTS_ON_CIRCLE, Test 4:  100 green dots on light blue.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(150, 200), 100)
    circle.fill_color = 'green'
    draw_points_on_circle(window, circle, 100, 'light blue')
    window.close_on_mouse_click()
...
def run_test_pizza():
    ...
    # ------------------------------------------------------------------
    title = 'PIZZA test 4:  50 thin black lines on yellow circle.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(150, 150), 125)
    circle.fill_color = 'yellow'
    circle.outline_thickness = 2
    pizza(window, circle, 50, 'black', 2)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(len(sequence)):
        line = rg.Line(circle.center, sequence[k])
        line.color = color
...
def run_test_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'POLYGON tests 3:  5 segments with thin yellow lines.'
    window = rg.RoseWindow(400, 400, title)

    circle = rg.Circle(rg.Point(150, 100), 90)
    circle.outline_thickness = 3
    polygon(window, circle, 5, 'yellow', 3)
    window.close_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(len(seq)):
        line = rg.Line(seq[k], seq[(k+1) % len(seq)])
        line.thickness = thickness
        line.color = color
        line.attach_to(window)
    window.render()
...
def run_test_fancy_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'FANCY POLYGON test 4:  51 red lines on black circle, hops = 25.'
    window = rg.RoseWindow(400, 400, title)

    circle = rg.Circle(rg.Point(250, 150), 125)
